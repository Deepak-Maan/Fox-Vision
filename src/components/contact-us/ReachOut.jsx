            ...
            <div className="container max-w-[1164px] px-3 mx-auto">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 mt-5  md:mt-10 lg:mt-[70px] xl:mt-[88px] gap-[30px]">
                    {reachOutData.map((data, index) => (
                        <Link to={data.link} target={data.target} rel='noreferrer' key={index} className={`max-w-[360px] mx-auto w-full max-h-[265px] group hover:translate-y-[-12px] lg:hover:translate-y-[-38px] duration-300 shadow-[0_4px_27px_0_#4B17E614] ${index === 0 ? "py-10" : index === 1 ? "py-10" : index === 2 ? "py-7 sm:col-span-2 lg:col-span-1 " : ""} `} >
                            ...
                            <p className=' text-darkblue text-center font-poppins font-semibold text-2xl !leading-[150%] pb-4 pt-6' >{data.heading}</p>
