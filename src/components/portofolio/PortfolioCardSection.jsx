            ...
            <div className='flex flex-wrap flex-row -mx-3'>
                {portfolioData.slice(0, showAll ? portfolioData.length : initialCardsToShow).map(item => (
                    <div key={item.id} className='lg:w-1/3 w-full sm:w-1/2 px-3 mb-4 md:mb-6'>
                        <div className='border-[1px] border-solid border-[#0000001A] group rounded-md p-[10px] sm:p-[15px_17px] h-full max-w-[364px] mx-auto lg:mx-0'>
                            <div className='max-w-[330px] max-h-[200px] sm:max-h-[311px] rounded-md overflow-hidden'><img src={item.image} alt={item.name} className='max-w-[330px] duration-300 group-hover:scale-[1.1] max-h-[200px] sm:max-h-[311px] w-full h-full rounded-md' /></div>
