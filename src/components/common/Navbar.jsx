...
const Navbar = () => {
  ...
  const getLinkClasses = (path) => {
    ...
    return `font-poppins font-normal relative before:absolute before:h-[3px] duration-300 transition-all before:transition-all before:bg-darkpurple before:start-[50%] before:translate-x-[-50%] before:bottom-[-5px] before:rounded-lg before:duration-300 ${isActive ? 'before:w-[26px] -webkit-text-stroke-width: 0.5px;' : 'before:w-0'} text-base text-darkblue ${isActive ? '-webkit-text-stroke-width: 0.5px;' : ''} hover:-webkit-text-stroke-width: 0.5px;`;
  };  
  return (
    <div className="pt-[19px]">
      <nav className="max-w-[1240px] mx-auto px-6 sm:px-[50px] h-[54px] sm:h-[74px] rounded-[150px] sm:py-[11px] flex items-center justify-between shadow-[0px_0px_13px_0px_#00000024] bg-white">
        ...
        >
          ...
          <li className="flex flex-col md:flex-row justify-center items-center gap-2 md:hidden md:gap-4 relative z-20">
            <span className="flex gap-3 mb-6">
              <Link to="tel:+1234567890" className="hover:translate-y-[-6px] transition-all duration-300 ease-linear">
                <Calling />
              </Link>
              <Link to="mailto:example@example.com" className="hover:translate-y-[-6px] transition-all duration-300 ease-linear">
                <Email />
              </Link>
            </span>
        ...
        <div className="flex items-center gap-2 md:gap-4 relative z-20">
          <Link to="tel:+1234567890" className="hover:translate-y-[-6px] hidden md:block transition-all duration-300 ease-linear">
            <Calling />
          </Link>
          <Link to="mailto:example@example.com" className="hover:translate-y-[-6px] hidden md:block transition-all duration-300 ease-linear">
            <Email />
          </Link>
